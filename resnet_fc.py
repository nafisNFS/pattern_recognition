...
# Load the pre-extracted ResNet features
nir_features = np.load(r"E:\RGB-NIR\resnet_nir_features.npy")
rgb_features = np.load(r"E:\RGB-NIR\resnet_rgb_features.npy")
labels = np.load(r"E:\RGB-NIR\labels.npy")

# Concatenate the features (NIR + RGB)
all_features = np.concatenate((nir_features, rgb_features), axis=1)  # Concatenate along the feature axis
...
# Define a simple classifier based on ResNet's fully connected layer
class ResNetClassifier(nn.Module):
    def __init__(self, input_dim, num_classes):
        super(ResNetClassifier, self).__init__()
        self.fc1 = nn.Linear(input_dim, 128)  # Adjust input dimension according to concatenated features
...
input_dim = X_train.shape[1]  # This should be the number of features in the concatenated vector
